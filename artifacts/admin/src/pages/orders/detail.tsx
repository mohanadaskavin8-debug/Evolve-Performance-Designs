...
import { useAdminGetOrder, getAdminGetOrderQueryKey, useAdminOrderAction } from '@workspace/api-client-react';
...
import { ArrowLeft, Package, Truck, AlertTriangle, ShieldCheck, CheckCircle2, RotateCcw, Box } from 'lucide-react';
              ...
              {order.shipments && order.shipments.length > 0 && (
                <div className="mt-8 space-y-4">
                  ...
                  {order.shipments.map(shipment => (
                    <div key={shipment.id} className="p-4 border rounded-lg bg-card shadow-sm flex flex-col sm:flex-row justify-between gap-4">
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          <CheckCircle2 className="w-4 h-4 text-emerald-500" />
                          <span className="font-bold">{shipment.carrier || 'Unknown Carrier'}</span>
                        </div>
                        {shipment.trackingNumber && (
                          <div className="font-mono text-sm text-primary flex items-center gap-2">
                            {shipment.trackingNumber}
                            {shipment.trackingUrl && (
                              <a href={shipment.trackingUrl} target="_blank" rel="noreferrer" className="text-xs underline text-muted-foreground hover:text-primary">
                                Track
                              </a>
                            )}
                          </div>
...
function FulfillOrderDialog({ orderId, disabled }: { orderId: number, disabled: boolean }) {
  ...
  const handleFulfill = () => {
    mutation.mutate({ action: 'fulfill', carrier, trackingNumber, trackingUrl } as any, {
      onSuccess: () => {
